...
write_block(uint32_t blockno)
{
	...
	if ((r = ide_write(blockno * BLKSECTS, addr, BLKSECTS)) < 0)
		panic("write_block: ide_write error %e\n", r);
	if ((r = sys_page_map(0, addr, 0, addr, PTE_USER)) < 0)
		panic("write_block: sys_page_map error %e\n", r);
}
...
read_bitmap(void)
{
	...
	int bm_block_num = super->s_nblocks / BLKBITSIZE + (super->s_nblocks % BLKBITSIZE == 0) ? 0 : 1;
	for (i = 0; i < bm_block_num; i++) {
		if ((r = read_block(2 + i, &blk)) < 0)
			panic("read_bitmap: read_block error %e at %d\n", r, 2 + i);
	}
...
file_flush(struct File *f)
{
	...
	int bno = ROUNDUP(f->f_size, BLKSIZE) / BLKSIZE;
	int i, r, diskbno;
	for (i = 0; i < bno; i++) {
