...
// lab 4 challenge ,set env priority
/*
static void sys_set_prio(envid_t envid, int p) {
	if (p <= ENV_PRIO_HIGHEST && p >= ENV_PRIO_LOWEST)
		envs[ENVX(envid)].env_prio = p;
	else panic("invalid priority");
}*/
...
sys_page_unmap(envid_t envid, void *va)
{
	...
	if (va >= (void *)UTOP || ((uint32_t)va & (PGSIZE - 1)) != 0)
		return -E_INVAL;
	int r;
	struct Env * env;
	if ((r = envid2env(envid, &env, 1)) < 0)
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	/*
	case SYS_set_prio:
		sys_set_prio((envid_t)a1, (int)a2);
		return 0;
	*/
	case SYS_exofork:
		return sys_exofork();
